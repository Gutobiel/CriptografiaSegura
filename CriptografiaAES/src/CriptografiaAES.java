...
public class CriptografiaAES {
	...
	private static String encriptar(String texto, String chave) throws Exception {
		...
		SecretKeySpec objChave = new SecretKeySpec(calcularHash(chave), "AES");
		Cipher objCifra = Cipher.getInstance("AES/CBC/PKCS5Padding");
		objCifra.init(Cipher.ENCRYPT_MODE, objChave, 
					  new IvParameterSpec("AugustoBonitao".getBytes("UTF-8")));
	...
	private static byte[] calcularHash(String texto) throws Exception {
		
		MessageDigest objResumidor = MessageDigest.getInstance("SHA-256");

		return objResumidor.digest(texto.getBytes("UTF-8"));
	}
